...
<script type="text/javascript">
	...
	CKEDITOR.replace( 'editor11',
	  ...
	  {if $about_images != ""}
	  {$img_cnt=$about_images|count}
	  {if $img_cnt>0}
		  $(".dropdown dt a#sample span").html("<img src=\"{$about_images[0][1]}\" /><span class=\"value\" style=\"display:none;\">{$about_images[0][0]}</span>");
		  {$img_cnt=$img_cnt-1}
	  {/if}
	  {if $img_cnt>0}
		  $(".dropdown dt a#sample2 span").html("<img src=\"{$about_images[1][1]}\" /><span class=\"value\" style=\"display:none;\">{$about_images[1][0]}</span>");
		  {$img_cnt=$img_cnt-1}
	  {/if}
	  {if $img_cnt>0}
		  $(".dropdown dt a#sample3 span").html("<img src=\"{$about_images[2][1]}\" /><span class=\"value\" style=\"display:none;\">{$about_images[2][0]}</span>");
		  {$img_cnt=$img_cnt-1}
	  {/if}
	  {if $img_cnt>0}
		  $(".dropdown dt a#sample4 span").html("<img src=\"{$about_images[3][1]}\" /><span class=\"value\" style=\"display:none;\">{$about_images[3][0]}</span>");
		  {$img_cnt=$img_cnt-1}
	  {/if}
	  {/if}
	  {literal}
...
if (window.XMLHttpRequest)
  ...
  xmlhttp.open("post","aboutus_update_db.php",true);
  ...
  xmlhttp.send("short_desc="+encodeURIComponent(CKEDITOR.instances.editor11.getData())+"&full_desc="+encodeURIComponent(CKEDITOR.instances.editor12.getData())+"&image_pics="+pics );
...
<br/>
لطفاً از نوار ابزار قرار گرفته در بالا برای ویرایش
	خلاصه ارتباط با ما
و معرفی کامل در بخش ارتباط با ما استفاده نمائید. <br />
توضیح مختصر برای ارتباط با ما :
<textarea id="editor11" class="ckeditor" cols="80" name="editor1" rows="10">{$about[0][0]}</textarea>
<br />
معرفی کلی شرکت ، سازمان و یا فرد :
<textarea id="editor12" class="ckeditor" cols="80" name="editor2" rows="10">{$about[0][1]}</textarea>
</div>
<div id="bottomSpace">
</div>
<p><input onclick="do_aboutus_update();" type="button" value="ویـــرایش اطلاعات ارتباط با ما" /> </p>
<div id="error_reporting" style="position: absolute;height:20px;right:100px;margin-top:-40px;margin-right:90px;">
